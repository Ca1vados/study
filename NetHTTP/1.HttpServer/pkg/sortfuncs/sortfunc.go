...
func BubbleSort(arr []int) []int {
	for i := 0; i < len(arr)-1; i++ { // загоняем все элементы слайса в цикл
		for j := 0; j < len(arr)-1-i; j++ { // загоняем во второй цикл все элементы слайса минус по сути количество предыдущих циклов
			if arr[j] > arr[j+1] { // сравниваем  попарно все элементы
